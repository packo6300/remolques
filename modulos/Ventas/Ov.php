...
if($_POST){
       ...
       $fecha2=$_POST['fecha2'];
       $sta=$_POST['st'];
       $sql="SELECT * FROM ovlistar where folio like '%$folio%' and cliente like '%$prov%' and (fecha between '$fecha1' and '$fecha2') and estatus like '%$sta%'  order by cliente,fecha;";
   }
   else{
       $fecha1=$hoy;
       $fecha2=$hoy;
       $sql="SELECT * FROM ovlistar where fecha between '$fecha1' and '$fecha2';";
   }
...
<script type="text/javascript">
   ...
   try{
        ...
        $.ajax({type: 'POST',url:"modulos/Ventas/Ov.php",data:s
        }).done(function (datos){
            $("#pant").html(datos);
        });
    }
...
function nvo(){
    try{
        ...
        $("#pant").load("modulos/Ventas/OvPropiedad.php");
    }
    catch(err){
        console.error(err);
    }
...
window.open("dom/output2pdf2.php?id="+id+"&url=Ventas/OvPDFTicket.php");
        ...
        <div class="row">
            <div>
                <label class="list-group-item"><span class="glyphicon glyphicon-chevron-left"></span>Ventas</label>
            </div>
                ...
                <table id="tbl_info" class="table table-striped">
                    <tr class="table table-hover">
                        ...
                        <td>
                            ...
                            <input value="<?php echo $folio; ?>" type="text" name="folio" onchange="recarga()"></td>
                        <td>
